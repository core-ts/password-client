...
function validateContact(contact, key, r, reg, showError) {
  if (showError) {
    if (isEmpty(contact)) {
      var msg = formatText(r.error_required, r[key]);
      showError(msg, 'contact');
      return false;
    }
    if (reg && !reg.test(contact)) {
      var msg = r.error_contact_exp;
      showError(msg, 'contact');
      return false;
    }
    return true;
...
function validateReset(user, confirmPassword, r, reg, showError) {
  if (showError) {
    if (isEmpty(user.username)) {
      ...
      var msg = formatText(r.error_required, r.passcode);
      showError(msg, 'passcode');
      return false;
    }
    else if (isEmpty(user.password)) {
      var msg = formatText(r.error_required, r.new_password);
      showError(msg, 'password');
      return false;
    }
    if (reg && !reg.test(user.password)) {
      var msg = formatText(r.error_password_exp, r.new_password);
      showError(msg, 'password');
      return false;
    }
...
function validateChange(user, confirmPassword, r, reg, showError) {
  if (showError) {
    ...
    if (isEmpty(user.currentPassword)) {
      var msg = formatText(r.error_required, r.current_password);
